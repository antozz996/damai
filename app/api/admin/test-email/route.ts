...
export async function POST(req: NextRequest) {
  try {
    ...
    const requestedCode = String(body.registrationCode || 'D26-TESTMAIL').replace(/[^A-Za-z0-9-]/g, '').slice(0, 30);
    const guest: Guest = { firstName: String(body.firstName || 'Ospite'), lastName: String(body.lastName || 'Test'), phone: String(body.phone || '+39 000 000 0000'), email, registrationCode: `${requestedCode || 'D26-TESTMAIL'}-${Date.now()}`, qrToken: randomUUID(), eventDate: /^\d{4}-\d{2}-\d{2}$/.test(String(body.eventDate || '')) ? String(body.eventDate) : '2026-10-30', startTime: /^\d{2}:\d{2}/.test(String(body.startTime || '')) ? String(body.startTime) : '16:00' };
